...
const DIMS: number = ModelRegistry.getDimensions(MODEL_NAME) || 384;

async function runParityDiagnostics(): Promise<void> {
  ...
  try {
    ...
    const mockAiProvider = {
      ...
      embed: async (text: string, dims: number) => ({
        vector: testVector,
        dims: dims || DIMS,
        model: MODEL_NAME,
    ...
    if (mongoDoc) {
      const mongoMag = Math.sqrt(mongoDoc.vector.reduce((s: number, v: number) => s + v*v, 0));
    ...
    await mongo.close();
    if ((pg as any).pool) await (pg as any).pool.end();
  }
}
