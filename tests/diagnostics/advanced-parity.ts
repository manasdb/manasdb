...
async function advancedParityDiagnostics(): Promise<void> {
  ...
  const store = async (id: string, vec: number[], text: string) => {
    const params = {
      rawText: text,
      filteredText: text,
  ...
  const formatResults = (res: any[]) => res.map(r => ({
