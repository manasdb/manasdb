...
async function runEndToEnd(): Promise<void> {
    ...
    console.log(chalk.bold("[INFO] MANASDB POLYGLOT END-TO-END SUITE (PG + MONGO)"));
    console.log(chalk.cyan("=====================================================\n"));

    let db: InstanceType<typeof ManasDB> | undefined;
    try {
        const projectName = 'poly_e2e_' + Date.now();
        db = new ManasDB({
            databases: [
        ...
        console.log(`  └─> Created ${absorbRes.chunks} chunks across ${absorbRes.inserted.length} databases.\n`);

        console.log(chalk.dim("  Waiting 6 seconds for vector indexes to synchronize..."));
        ...
        for (const item of E2E_QUERIES) {
            ...
            if (results.length > 0) {
                ...
                if (passed) {
                    ...
                    console.log(`  ${chalk.dim('Score: ' + results[0].score + ' | Database: ' + results[0].metadata.database)}`);
                    score++;
                } else {
                ...
                console.log(chalk.dim(`  Trace: Cache=${(results as any)._trace.cacheHit}, RRFMerges=${(results as any)._trace.rrfMerged}`));
            } else {
                console.log(`  ${chalk.red('[FAIL]')} No chunks returned. Check indexes.`);
            }
        }
        ...
        if (score === E2E_QUERIES.length) {
            console.log('🎉 ALL POLYGLOT E2E TESTS PASSED!');
            setTimeout(() => process.exit(0), 500);
        } else {
            console.log(chalk.red("❌ SOME POLYGLOT E2E TESTS FAILED."));
            process.exit(1);
        }
