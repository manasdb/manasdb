...
async function runRandomTextTest() {
    ...
    try {
        ...
        const t0 = Date.now();
        const absorbRes = await db.absorb(ARTICLE_TEXT);
        
        console.log(`[STATUS] Absorb Complete in ${Date.now() - t0}ms`);
        console.log(`[STATUS] Processed ${absorbRes.chunks} chunks.\n`);

        console.log(`[INFO] Waiting 8 seconds for Atlas vector index to catch up...`);
        ...
        for (const item of QUERIES) {
            ...
            const results = await db.recall(item.text, { mode: 'document', limit: 3, minScore: 0.05 } as any);
            ...
            if (results.length > 0) {
                ...
                if (passed) {
                    ...
                    console.log(`  [SCORE] Top Match Score: ${results[0].score?.toFixed(4) || 'N/A'}`);
                    score++;
                } else {
                    console.log(`  [FAIL] Expected phrase "${item.expected}" not found in top chunk.`);
                    console.log(`  [RETURNED CHUNK] "${topText}"`);
                }
            } else {
