...
async function main(): Promise<void> {
  ...
  console.log(resultsNoMMR.map((r: any) => r.metadata?.matchedChunk || r.contentDetails?.[0]?.text));
  ...
  console.log(resultsWithMMR.map((r: any) => r.metadata?.matchedChunk || r.contentDetails?.[0]?.text));

  const mmrTexts = resultsWithMMR.map((r: any) => r.metadata?.matchedChunk || r.contentDetails?.[0]?.text || '');
  if (mmrTexts.some((t: string) => String(t).includes('Bananas'))) {
