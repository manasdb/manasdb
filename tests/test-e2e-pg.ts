...
async function runEndToEnd() {
    ...
    console.log(chalk.bold("[INFO] MANASDB END-TO-END (E2E) TEST SUITE (POSTGRES)"));
    console.log(chalk.cyan("=====================================================\n"));

    let db: ManasDB | undefined;
    try {
        ...
        const t0 = Date.now();
        const absorbRes = await db.absorb(ARTICLE_TEXT, { maxTokens: 80 } as any);
        
        console.log(chalk.green(`[SUCCESS] Stored accurately in ${Date.now() - t0}ms`));
        console.log(`  └─> Created ${absorbRes.inserted[0]?.chunksInserted || absorbRes.chunks} chunks mapped to 1 parent document.\n`);
        ...
        for (const item of E2E_QUERIES) {
            ...
            const results = await db.recall(item.text, { mode: item.mode, limit: 3 } as any);
            
            if (results.length > 0) {
                const topText = results[0].metadata?.matchedChunk || results[0].text || '';
                ...
                console.log(chalk.dim(`  Trace: Cache=${(results as any)._trace?.cacheHit}`));
            } else {
                console.log(`  ${chalk.red('[FAIL]')} No chunks returned.`);
            }
        }
