...
async function runFeaturesTest(): Promise<void> {
    ...
    try {
        ...
        const assert = (condition: boolean, successMsg: string, failMsg: string): void => {
            total++;
            if (condition) {
        ...
        const parentMongoId = absorb1.inserted.find((i: any) => i.database === 'mongodb').contentId;
        ...
        assert(
            (recall1 as any)._trace.cacheHit === false,
