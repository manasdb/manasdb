...
export class CostCalculator {
    static PRICING_TABLE: Record<string, number> = {
        'openai': 0.02,
     ...
     */
    static calculate(tokens: number, model: string): number {
        let pricePer1M = this.PRICING_TABLE[model];
        if (pricePer1M === undefined) {
             const key = Object.keys(this.PRICING_TABLE).find(k => model && model.includes(k));
     ...
     */
    static estimateSavings(tokens: number, model: string): number {
     ...
     */
    static calculateProjectedAnnual(monthlySavings: number): number {
     ...
     */
    static estimateAbsorbCost(text: string, model: string): { tokens: number; costUSD: number; model: string } {
     ...
     */
    static estimateRecallCost(query: string, model: string): { tokens: number; costUSD: number; model: string } {
