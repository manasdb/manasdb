...
export class ManasDB {
  ...
  public databaseDrivers: any[];
  public targetDims?: number;
  public aiProvider: any;
  ...
  async migrateTo(targetConfig: ManasDBConfig): Promise<{ migratedCount: number }> {
    console.log(`[ManasDB] Starting Migration...`);
  ...
  async clearAll(): Promise<void> {
    if (this.debug) {
      ...
      console.warn('[ManasDB] _manas_telemetry was NOT cleared.');
    }
    await Promise.all(this.databaseDrivers.map(async driver => {
  ...
  async clearTelemetry(): Promise<void> {
    console.warn('[ManasDB] Clearing telemetry will permanently delete your cost savings history and performance metrics.');
    await Promise.all(this.databaseDrivers.map(async driver => {
